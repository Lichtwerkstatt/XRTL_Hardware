...
const int stepperOneA = 14;
const int stepperOneB = 13;
const int stepperOneC = 12;
const int stepperOneD = 15;
String stepperOneName = "top";
AccelStepper stepperOne(AccelStepper::HALF4WIRE, stepperOneA, stepperOneB, stepperOneC, stepperOneD);

const int stepperTwoA = 2;
const int stepperTwoB = 5;
const int stepperTwoC = 4;
const int stepperTwoD = 16;
        ...
        if (component == componentID) {
          ...
          if (receivedPayload["command"].is<JsonObject>()) {
            ...
            if (command == "restart") {
              ...
              pos.one = stepperOne.currentPosition();
              pos.two = stepperTwo.currentPosition();
              EEPROM.put(0,pos);
              EEPROM.commit();
              WiFi.disconnect();
              ESP.restart();
...
void setup() {
  ...
  EEPROM.begin(512);
  EEPROM.get(0,pos);
  stepperOne.setCurrentPosition(pos.one);
  stepperTwo.setCurrentPosition(pos.two);
